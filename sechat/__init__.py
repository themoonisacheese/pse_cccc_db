"""Vendored copy of the `sechat` library (upstream: https://github.com/nvua/sechat, v1.1.2).

The upstream project is no longer maintained, so we vendor it here to apply a
local fix: a malformed (non-string) websocket frame used to crash the room
listener thread, silently leaving the ingest daemon deaf. See
`tests/test_vendored_sechat.py` for the regression test.

Only change vs upstream: the `Room.run` receive loop now logs bad frames safely
(`%r`) and `continue`s instead of concatenating a str with a (possibly dict)
payload and dying.
"""
    ...
    while self.running:
      ...
      if data is not None and data != "":
        try:
          ...
          # Vendored fix: `data` may be a dict (non-string frame) rather than a
          # JSON string.  The upstream code crashed here because it tried to
          # concatenate a str with the (possibly dict) payload.  Log safely and
          # keep the listener alive instead of killing the room thread.
          self.logger.warning("Recived corrupted data: %r", data)
          continue
        else:
          self.lastPing = time.time()
          self.process(data)
